#!/usr/bin/env python3
"""Build and sign the immutable TeamManager Pocket R2 repair authority."""
...
R2_RUNTIME_ROOT = "pocket-runtime-win-cpu-v2.1.0-r2/"
R2_VOICE_ROOT = "pocket-voice-alba-v2.1.0-r2/"
R2_RELEASE = "pocket-tts-v2.1.0-r2"
R2_RUNTIME_FILE = "pocket-runtime-win-cpu-v2.1.0-r2.zip"
R2_VOICE_FILE = "pocket-voice-alba-v2.1.0-r2.zip"
R2_REVISION = "teammanager-pocket-r2"
...
def build_runtime(base_runtime: Path, worker: Path, output: Path) -> None:
    ...
    for marker in REQUIRED_WORKER_MARKERS:
        if marker not in worker_text:
            raise ValueError(f"worker is missing required R2 marker: {marker}")
    ...
    with zipfile.ZipFile(base_runtime) as source, zipfile.ZipFile(
        ...
        for source_info in source.infolist():
            ...
            target_name = R2_RUNTIME_ROOT + suffix
...
def build_voice(alba: Path, output: Path) -> None:
    ...
    with zipfile.ZipFile(output, "w", allowZip64=True) as archive:
        archive.writestr(
            zip_info(R2_VOICE_ROOT + "THIRD_PARTY_NOTICES.md"),
            notice,
            compress_type=zipfile.ZIP_DEFLATED,
            compresslevel=9,
        )
        archive.writestr(
            zip_info(R2_VOICE_ROOT + "upstream-source.json"),
            provenance,
            compress_type=zipfile.ZIP_DEFLATED,
            compresslevel=9,
        )
        state_info = zip_info(R2_VOICE_ROOT + "voice/alba.safetensors")
...
def update_manifest(
    ...
    bundle["package_revision"] = R2_REVISION

    runtime_asset = bundle["runtime"]
    runtime_asset.update(
        {
            "package_revision": R2_REVISION,
            "release_tag": R2_RELEASE,
            "file": R2_RUNTIME_FILE,
            "url": f"{{base_url}}/Max/teammanager-models/releases/download/{R2_RELEASE}/{R2_RUNTIME_FILE}",
            "sha256": sha256(runtime),
            "size_bytes": runtime.stat().st_size,
            "extracted_root": R2_RUNTIME_ROOT.rstrip("/"),
        }
    )
    ...
    voice_asset.update(
        {
            ...
            "release_tag": R2_RELEASE,
            "file": R2_VOICE_FILE,
            "url": f"{{base_url}}/Max/teammanager-models/releases/download/{R2_RELEASE}/{R2_VOICE_FILE}",
            "sha256": sha256(voice),
            "size_bytes": voice.stat().st_size,
            "extracted_root": R2_VOICE_ROOT.rstrip("/"),
            "attribution": (
    ...
    manifest["checksums"]["files"][R2_RUNTIME_FILE] = sha256(runtime)
    manifest["checksums"]["files"][R2_VOICE_FILE] = sha256(voice)
    output.write_bytes(
...
def publish(args: argparse.Namespace) -> None:
    ...
    runtime = args.output_dir / R2_RUNTIME_FILE
    voice = args.output_dir / R2_VOICE_FILE
    unsigned_manifest = args.output_dir / "manifest.r2.json"
